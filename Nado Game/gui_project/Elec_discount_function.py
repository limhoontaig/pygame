...
def add_file():
    files = filedialog.askopenfilename(title="엑셀 데이타 파일을 선택하세요", \
        filetypes=(("PNG 파일", "*.xls",'*.xlsx'), ("모든 파일", "*.*")), \
...
btn_welfare_path = Button(welfare_frame, text="찾아보기", width=10, command=browse_dest_path)
...
btn_kind_welfare_path = Button(kind_welfare_frame, text="찾아보기", width=10, command=browse_dest_path)
btn_kind_welfare_path.pack(side="right", padx=5, pady=5)

# 저장 경로 프레임
path_frame = LabelFrame(root, text="XPERP 할인자료 파일 저장경로")
